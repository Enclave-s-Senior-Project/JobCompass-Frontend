...
export default function JobRowDashboardUser({ temp, handleViewDetails }: props) {
    return (
        <motion.tr initial="hidden" animate="visible" whileHover={{ backgroundColor: '#f9fafb' }}>
            <td className="whitespace-nowrap px-6 py-4">{temp.job.name}</td>
            <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                <motion.span whileHover={{ scale: 1.05 }} className={`ml-4 rounded-md px-2 py-1 text-xs`}>
                    {temp.status}
                </motion.span>
            </td>
            <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{toFormattedDate(temp?.createdAt)}</td>
